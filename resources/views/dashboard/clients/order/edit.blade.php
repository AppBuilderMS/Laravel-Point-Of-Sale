...
@section('content')
    <section class="content-header" style="margin-bottom: 20px">
        ...
        <ol class="breadcrumb">
            ...
            {{--        <li><a href="{{route('dashboard.clients.orders')}}">@lang('site.clients')</a></li>--}}
            <li class="active">@lang('site.orders_edit')</li>
        </ol>
    </section>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <a href=""
                                                                   ...
                                                                   class="btn {{in_array($product->id, $order->products->pluck('id')->toArray()) ? 'btn-default disabled btn-sm' : 'btn-success btn-sm add-product-btn'}}">
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        ...
                        <form action="{{ route('dashboard.clients.orders.update', ['client'=>$order->client->id, 'order'=>$order->id]) }}" method="post">
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody class="order-list">
                                    ...
                                    @foreach($order->products as $product)
                                        <tr>
                                            <td>{{$product->name}}</td>
                                            <td><input type="number" name="products[{{$product->id}}][quantity]" data-price="{{number_format($product->sale_price,2)}}" class="form-control product-quantity" min="1" value="{{$product->pivot->quantity}}"></td>
                                            <td class="product-price">{{number_format($product->sale_price * $product->pivot->quantity, 2)}}</td>
                                            <td>
                                                <button type="submit" class="btn btn-danger btn-sm remove-product-btn" data-id="{{$product->id}}"><span class="fa fa-trash"></span></button>
                                            </td>
                                        <tr/>
                                    @endforeach
                            ...
                            <button class="btn btn-primary btn-block" ><i class="fa fa-plus"></i> @lang('site.edit_order')</button>
