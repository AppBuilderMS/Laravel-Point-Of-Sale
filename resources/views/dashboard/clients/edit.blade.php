        ...
        <form action="{{route('dashboard.clients.update', $client->id)}}" method="POST">
            ...
            @for ($i = 0; $i < 2; $i++)
                <div class="form-group" style="margin-bottom: 10px">
                    ...
                    <input type="text" name="phone[]" id="phone" class="form-control" value="{{$client->phone[$i] ?? ""}}">
                </div>
            @endfor

            <div class="form-group" style="margin-bottom: 10px">
                <label for="address">@lang('site.address')</label>
                <textarea type="text" name="address" id="address" class="form-control">{{$client->address}}</textarea>
            </div>
